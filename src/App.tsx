...
export default function App() {
  ...
  return (
    <div className="flex h-screen bg-bento-bg text-slate-200 font-sans overflow-hidden p-6 gap-6 flex-col">
      <header className="h-20 flex items-center justify-between px-6 bg-bento-card border border-bento-border rounded-2xl shadow-xl shrink-0">
        <div className="flex items-center gap-4">
          <div className="h-10 w-10 bg-indigo-600 rounded-lg flex items-center justify-center font-bold text-white shadow-lg">Z</div>
          <div>
            <h1 className="text-xl font-bold tracking-tight">SPRINTAZ <span className="text-slate-500 font-normal">/ {activeSprint ? activeSprint.name : 'No Sprint Selection'}</span></h1>
            <p className="text-[10px] uppercase font-bold text-slate-500 tracking-widest mt-0.5">CIFP ZONZAMAS</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-4 border-l pl-6 border-bento-border">
            ...
            {user.photoURL ? (
              <img src={user.photoURL} className="w-10 h-10 rounded-full border border-indigo-500/30 ring-2 ring-indigo-500/10" alt={user.name} referrerPolicy="no-referrer" />
            ) : (
        ...
        <main className="flex-1 min-w-0 flex flex-col">
          <AnimatePresence mode="wait">
            {activeSprint ? (
              ...
              <div className="h-full bg-bento-card border border-bento-border rounded-2xl flex flex-col items-center justify-center text-slate-500 gap-4 shadow-lg">
                <Layers className="w-16 h-16 opacity-10" />
                <p className="text-lg font-medium opacity-50">Selecciona un sprint para visualizar el tablero</p>
              </div>
            )}
          </AnimatePresence>
        </main>
      ...
      <footer className="h-12 bg-bento-card border border-bento-border rounded-2xl flex items-center px-6 gap-4 shadow-inner shrink-0">
        <div className="flex items-center gap-2 text-indigo-400">
          <div className="w-2 h-2 rounded-full bg-indigo-500"></div>
          <span className="text-[10px] font-bold uppercase tracking-widest">Sistema:</span>
        </div>
        <p className="text-xs text-slate-400 flex-1 truncate italic">
          Conectado como <span className="text-slate-300 font-bold">{user.email}</span> · Local: Lanzarote, Canarias
        </p>
        <div className="flex items-center gap-2">
          <div className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse"></div>
          <span className="text-[10px] font-mono text-slate-600 uppercase tracking-tighter">Live Database Connected</span>
        </div>
      </footer>
    </div>
  );
